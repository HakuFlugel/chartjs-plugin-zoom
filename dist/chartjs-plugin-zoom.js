...
function resolveOptions(chart, options) {
	...
	if (zoomEnabled && dragEnabled) {
		...
		node.addEventListener('touchstart', props._touchStartHandler);//
		node.ownerDocument.addEventListener('mouseup', props._mouseUpHandler);
		node.ownerDocument.addEventListener('touchend', props._touchEndHandler);//
	} else {
		node.removeEventListener('mousedown', props._mouseDownHandler);
		node.removeEventListener('touchstart', props._touchStartHandler);//
		node.removeEventListener('mousemove', props._mouseMoveHandler);
		node.removeEventListener('touchmove', props._touchMoveHandler);//
		node.ownerDocument.removeEventListener('mouseup', props._mouseUpHandler);
		node.ownerDocument.removeEventListener('touchend', props._touchEndHandler);//
	}
}
...
var zoomPlugin = {
	...
	beforeInit: function(chartInstance, pluginOptions) {
		...
		chartInstance.$zoom._touchStartHandler = function(event) {
			node.addEventListener('touchmove', chartInstance.$zoom._touchMoveHandler);
			chartInstance.$zoom._dragZoomStart = event;
		};
		...
		chartInstance.$zoom._touchMoveHandler = function(event) {
			if (chartInstance.$zoom._dragZoomStart) {
				chartInstance.$zoom._dragZoomEnd = event;
			}
		};
		...
		chartInstance.$zoom._touchEndHandler = function(event) {
			if (!chartInstance.$zoom._dragZoomStart) {
				return;
			}
			node.removeEventListener('touchmove', chartInstance.$zoom._touchMoveHandler);

			var beginPoint = chartInstance.$zoom._dragZoomStart;
			
			var offsetX = beginPoint.target.getBoundingClientRect().left;
			var startX = Math.min(beginPoint.changedTouches[0].clientX, event.changedTouches[0].clientX) - offsetX;
			var endX = Math.max(beginPoint.changedTouches[0].clientX, event.changedTouches[0].clientX) - offsetX;

			var offsetY = beginPoint.target.getBoundingClientRect().top;
			var startY = Math.min(beginPoint.changedTouches[0].clientY, event.changedTouches[0].clientY) - offsetY;
			var endY = Math.max(beginPoint.changedTouches[0].clientY, event.changedTouches[0].clientY) - offsetY;
	...
	beforeDatasetsDraw: function(chartInstance) {
		...
		if (chartInstance.$zoom._dragZoomEnd) {
			...
			if (beginPoint.changedTouches) {
				if (directionEnabled(chartInstance.$zoom._options.zoom.mode, 'x', chartInstance)) {
					var offsetX = beginPoint.target.getBoundingClientRect().left;
					startX = Math.min(beginPoint.changedTouches[0].clientX, endPoint.changedTouches[0].clientX) - offsetX;
					endX = Math.max(beginPoint.changedTouches[0].clientX, endPoint.changedTouches[0].clientX) - offsetX;
				}

				if (directionEnabled(chartInstance.$zoom._options.zoom.mode, 'y', chartInstance)) {
					var offsetY = beginPoint.target.getBoundingClientRect().top;
					startY = Math.min(beginPoint.changedTouches[0].clientY, endPoint.changedTouches[0].clientY) - offsetY;
					endY = Math.max(beginPoint.changedTouches[0].clientY, endPoint.changedTouches[0].clientY) - offsetY;
				}
			} else {
				if (directionEnabled(chartInstance.$zoom._options.zoom.mode, 'x', chartInstance)) {
					var offsetX = beginPoint.target.getBoundingClientRect().left;
